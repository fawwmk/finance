...
export interface ExchangeRates {
  ...
  /** Откуда взяты на самом деле. */
  source: RateSource;
  /**
   * Какой источник просили. Отличается от `source`, когда банк не ответил
   * и мы откатились на ЦБ. Без этого поля приложение считало бы такие курсы
   * «не тем источником» и дёргало сеть при каждом открытии, без остановки.
   */
  requestedSource?: RateSource;
  /** Для банка: наличные или безнал — курсы отличаются заметно. */
  cashMode?: CashMode;
  /** Какой режим просили (см. requestedSource). */
  requestedCashMode?: CashMode;
